...
class PurgeCog(commands.Cog):
    ...
    @app_commands.describe(amount="一括削除するメッセージ数", reason="一括削除した理由", ephemeral="表示するかどうか")
    @app_commands.checks.has_permissions(manage_messages=True)
    @app_commands.checks.has_permissions(read_message_history=True)
    @app_commands.checks.bot_has_permissions(manage_messages=True)
    @app_commands.checks.bot_has_permissions(read_message_history=True)
    async def purge(self, interaction: discord.Interaction, amount: app_commands.Range[int, 1, 100], reason: str = "理由が入力されてません", ephemeral: bool = True):
        try:
            audit_reason = f"実行者: {interaction.user} | 理由: {reason}"     
            await interaction.channel.purge(limit=amount, reason=audit_reason, delete_message_days=days)
